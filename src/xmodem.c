...
void xmodem_receive(void) {
  ...
  xmodem_actual_flash_address = APP_START_ADDR;
...
uint8_t WORD_ALIGNED_ATTR received_packet_data[X_PACKET_1024_SIZE];
...
static xmodem_status xmodem_handle_packet(uint8_t const header)
{
  ...
  if (UART_ERROR == serial_read_timeout(&received_packet_number[0u], sizeof(received_packet_number), TIMEOUT))
    return X_ERROR_UART;
  if (UART_ERROR == serial_read_timeout(&received_packet_data[0u], size, TIMEOUT))
    return X_ERROR_UART;
  if (UART_ERROR == serial_read_timeout(&received_packet_crc[0u], sizeof(received_packet_crc), TIMEOUT))
    return X_ERROR_UART;
  ...
  /* error handling. */
  if (xmodem_packet_number != received_packet_number[X_PACKET_NUMBER_INDEX])
  {
  ...
  if (false == x_first_packet_received)
  {
    /* Reset WD */
    fwdgt_counter_reload();
    if (0 > flash_storage_erase(APP_START_ADDR, APP_SIZE))
    {
  ...
  if (0 > flash_storage_write(xmodem_actual_flash_address,
...
static xmodem_status xmodem_error_handler(uint8_t * const error_number)
{
  ...
  if ((*error_number) >= X_MAX_ERRORS) {
    /* Graceful abort. */
    (void)serial_write(X_CAN);
    (void)serial_write(X_CAN);
    return X_ERROR;
  }

  /* Otherwise send a NAK for a repeat. */
  (void)serial_write(X_NAK);
  return X_OK;
}
